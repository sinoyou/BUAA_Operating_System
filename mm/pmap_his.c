...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
	...
	//printf("pgtable:%X\npgdir_entryp:%X\n*pgdir_entryp:%X\n",pgtable,pgdir_entryp,*pgdir_entryp);
	//printf("pgdir_entryp:%X\nPDX(va):%d\n*pgdir_entryp:%x\npgtable:%x\n",pgdir_entryp,PDX(va),*pgdir_entryp,pgtable);
...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
	...
	if(size % BY2PG != 0){
		panic("什么情况，Boot_Map_Segment里面大小未对齐\n");
		return;
	}
...
page_init(void)
{
     ...
     * filed to 1) */
	u_short i;
	/*for(i=PADDR(end);i<PADDR(freemem);i+=BY2PG){
		pa2page(i)->pp_ref=1;
	}
	for(i=PADDR(freemem);i<maxpa;i+=BY2PG){
		pa2page(i)->pp_ref=0;
		LIST_INSERT_HEAD(page_free_list,pa2page(i),pp_link);
	}*/
	extern char end[]; 
	u_long used=freemem - ULIM;//(u_long)end;
	for(i=0;used;i++){
		pages[i].pp_ref=1;
		used-=BY2PG;
	}
	...
	if((*pgdir_entryp & PTE_V)==0){
		if(create){
			if(page_alloc(&ppage) == -E_NO_MEM){
				...
				*pgdir_entryp = page2pa(ppage)|PTE_V;//这个地方要不要加入PTE_R?
				ppage->pp_ref++;
			}
		}else{
			*ppte = NULL;
